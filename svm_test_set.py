...
if __name__ == '__main__':
    ...
    for i in range(len(k)):
        ...
        model = svm.SVC(probability=True,
                        degree=k[i]).fit(X_LS_features, np.ravel(y_LS_pairs))
    ...
    best_model = svm.SVC(probability=True, degree=k[best])
    ...
    final_model = svm.SVC(probability=True, degree=k[best])
    final_model.fit(X_LS_VS_TS_features,np.ravel(y_LS_VS_TS_pairs))

...
if __name__ == '__main__':
    ...
    model = svm.SVC(probability=True, degree=5).fit(X_LS_features, np.ravel(y_LS_pairs))
    y_hat = model.predict_proba(X_VS_features)[:, 1]

    y_hat = output_reconstruction(y_hat)
    scores = accuracy_score(y_VS, y_hat)
    ...
    print('\nTraining on LS+VS...')
    model = model.fit(X_LS_VS_features, np.ravel(y_LS_VS_pairs))
    ...
    final_model = svm.SVC(probability=True, degree=5)
                          .fit(X_LS_VS_TS_features,np.ravel(y_LS_VS_TS_pairs))
